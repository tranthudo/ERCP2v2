...
glmUnitize(GLMmodel* model)
{
    ...
    cy = (maxy + miny) / 2.0;
    cz = (maxz + minz) / 2.0;
	model->centerPoint[0] = cx;
	model->centerPoint[1] = cy;
	model->centerPoint[2] = cz;
...
glmFacetNormals(GLMmodel* model)
{
    ...
    for (i = 0; i < model->numtriangles; i++) {
        ...
        glmCross(v, u, &model->facetnorms[3 * (i+1)]);  // change to v,u order if wanna draw outside of the object
...
glmReadOBJ(const char* filename)
{
	...
	//glmUnitize(model);// Scale to cube
